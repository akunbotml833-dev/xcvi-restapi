...
module.exports = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn('Scraping error, using fallback values:', scrapeError.message);
      // Use fallback font families if scraping fails
      fontFamilies = {
        'compactablack': { name: 'Impact' },
        'drukcondsuper': { name: 'Impact' },
        'arial_narrowregular': { name: 'Arial Narrow' },
        'timesregular': { name: 'Times New Roman' }
      };
    }
    
    // Generate image based on format
    if (cleanFormat === 'svg') {
      return await generateSVG(res, cleanText, cleanTheme, fontFamilies, req);
    } else {
      return await generateCanvasImage(res, cleanText, cleanTheme, cleanFormat, fontFamilies, fontSizeMultiplier, req);
    }
...
async function generateSVG(res, text, theme, fontFamilies, req) {
  try {
    const width = 800;
    const height = 800;
...
async function generateCanvasImage(res, text, theme, format, fontFamilies, fontSizeMultiplier, req) {
  try {
    const width = 800;
    const height = 800;
